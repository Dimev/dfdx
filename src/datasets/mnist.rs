...
#[derive(Debug)]
pub enum MnistDownloadError {
    IoError(std::io::Error),
    CurlError(curl::Error),
    Md5Mismatch,
}

impl std::fmt::Display for MnistDownloadError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("{:?}", self))
    }
}

impl std::error::Error for MnistDownloadError {}

impl From<std::io::Error> for MnistDownloadError {
    fn from(e: std::io::Error) -> Self {
        Self::IoError(e)
    }
}

impl From<curl::Error> for MnistDownloadError {
